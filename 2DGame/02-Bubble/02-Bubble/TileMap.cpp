...
bool TileMap::collisionMoveLeft(const glm::ivec2 &pos, const glm::ivec2 &size, int* posX, int displacement) const
{
	...
	for(int y=y0; y<=y1; y++)
	{
		if (map[y * mapSize.x + x] == 1 || map[y * mapSize.x + x] == 2 || map[y * mapSize.x + x] == 3 || map[y * mapSize.x + x] == 41 || map[y * mapSize.x + x] == 49)
		{
			return false;
		}
...
bool TileMap::collisionMoveDown(const glm::ivec2 &pos, const glm::ivec2 &size, int *posX, int *posY, int initialX, int initialY, bool bGodMode) const
{
	int x0, x1, y;
